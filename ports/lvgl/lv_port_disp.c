...
static void lcd_fb_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    uint32_t addr = (uint32_t) (LTDC_LAYER(&LtdcHandle, 0)->CFBAR) + 2 * (LV_HOR_RES_MAX * area->y1 + area->x1);

    DMA2D->CR = 0x00000000UL | (1 << 9);    //enbale TCIE
    DMA2D->FGMAR = (uint32_t) (uint16_t*) (color_p);
    DMA2D->OMAR = (uint32_t) addr;

    DMA2D->FGOR = 0;                   //front offset
    DMA2D->OOR = OffLineSrc;        //output offset
...
{
    rt_err_t result;
    lv_color_t *lv_disp_buf1, *lv_disp_buf2;

    lv_disp_buf1 = (lv_color_t *)LV_MEM_CUSTOM_ALLOC(DISP_BUF_SIZE*sizeof(lv_color_t));
    lv_disp_buf2 = (lv_color_t *)LV_MEM_CUSTOM_ALLOC(DISP_BUF_SIZE*sizeof(lv_color_t));
